...
function buildCtrlBars() {{
  ...
  tabs.forEach(function(tab) {{
    ...
    if (!el) return;
    // Build buttons using DOM to avoid quote escaping issues
    const bar = document.createElement('div');
    bar.className = 'ctrl-bar';

    // Facility group
    const fg = document.createElement('div');
    fg.className = 'ctrl-group';
    [['all','All Facilities'],['fw','Fort Wayne'],['wl','Waterloo']].forEach(function(pair) {{
      const bf = document.createElement('button');
      bf.className = 'ctrl-btn' + (FAC===pair[0]?' active':'');
      bf.textContent = pair[1];
      bf.onclick = (function(f,t){{ return function(){{ setFac(f,t); }}; }})(pair[0], tab);
      fg.appendChild(bf);
    }});

    // Separator
    const s1 = document.createElement('div');
    s1.className = 'ctrl-sep'; s1.textContent = '|';

    // Period group
    const pg = document.createElement('div');
    pg.className = 'ctrl-group';
    [['7d','7D'],['30d','30D'],['all','ALL']].forEach(function(pair) {{
      const bp = document.createElement('button');
      bp.className = 'ctrl-btn' + (PERIOD===pair[0]?' active':'');
      bp.textContent = pair[1];
      bp.onclick = (function(p,t){{ return function(){{ setPeriod(p,t); }}; }})(pair[0], tab);
      pg.appendChild(bp);
    }});

    // Separator
    const s2 = document.createElement('div');
    s2.className = 'ctrl-sep'; s2.textContent = '|';

    // Granularity group
    const gg = document.createElement('div');
    gg.className = 'ctrl-group';
    [['day','DAY'],['week','WEEK'],['month','MONTH']].forEach(function(pair) {{
      const bg = document.createElement('button');
      bg.className = 'ctrl-btn' + (GRAN===pair[0]?' active':'');
      bg.textContent = pair[1];
      bg.onclick = (function(g,t){{ return function(){{ setGran(g,t); }}; }})(pair[0], tab);
      gg.appendChild(bg);
    }});
...
function renderNotes() {{
  ...
  document.getElementById('notes-tbody').innerHTML = ns.map(function(n,i) {{
    const preview = n.n.length > 80 ? n.n.slice(0,80)+'\u2026' : n.n;
    const hasMore = n.n.length > 80;
    const facCls  = n.fa==='Fort Wayne'?'fac-fw':'fac-wl';
    const facLbl  = n.fa==='Fort Wayne'?'FW':'WL';
    const expandSpan = hasMore ? '<span class="note-expand-icon">\u25bc expand</span>' : '';
    const fullSpan   = hasMore ? '<span class="note-full">' + n.n + '</span>' : '';
    return '<tr class="note-row" id="note-' + i + '" onclick="toggleNote(' + i + ')">'
      ...
      + '<td style="vertical-align:top;padding-top:10px"><span class="' + facCls + '">' + facLbl + '</span></td>'
      + '<td class="note-cell"><span class="note-preview">' + preview + expandSpan + '</span>' + fullSpan + '</td>'
      + '</tr>';
  }}).join('');
}}
